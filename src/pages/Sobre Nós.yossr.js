...
$w.onReady(async function () {
    let currentPage = 1;
    let itemsPerPage = wixWindowFrontend.formFactor === "Mobile" ? 4 : 16;
    ...
    function loadProducts(page) {
        ...
        .find()
        .then((results) => {
            const storeProducts = results.items;
            let totalProducts = results.totalCount;
            let totalPages = Math.ceil(totalProducts / itemsPerPage);

            $w('#repeaterProducts').data = storeProducts;
            $w('#paginationProducts').totalPages = totalPages;
        });
    }

    $w('#paginationProducts').currentPage = 1;
    $w('#paginationProducts').onChange((event) => {
    ...
    $w('#repeaterProducts').onItemReady(($item, itemData) => {
        ...
        // let productOptions = [];
        // itemData.productOptions[0].choices.forEach(choice => {
        //     productOptions.push({
        ...
        // $item('#dropdownOptions').options = productOptions;
        // $item('#dropdownOptions').onChange((event) => {
        //     optionProductSelected = event.target.selectedIndex;
        //     const variantPrice = itemData.variants[optionProductSelected].variant.convertedPriceData.formatted.discountedPrice;
        //     $item('#txtProductOfferPrice').text = variantPrice;
        // })
        ...
        if ((itemData.stock.inventoryStatus == "IN_STOCK") || (itemData.stock.inventoryStatus == "PARTIALLY_OUT_OF_STOCK")) {
            const addToCartAction = () => {
                ...
                if (itemData.productOptions.length > 0) {
                    let optionName = itemData.productOptions[0].name;
                    let selectedOptionValue = itemData.productOptions[0].choices[optionProductSelected].description;
                    itemToAdd = [{
